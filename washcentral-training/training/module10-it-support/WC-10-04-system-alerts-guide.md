...
- **The expected content is parked, not guessed.** The curriculum describes a KPI band, an **Info / Warning / Error / Critical** severity split, a dated alerts table, and infrastructure alert rows for this page. None of it is observable in the July 9 capture, so it is held as a new parked item (see structured findings) rather than written up as if seen.
