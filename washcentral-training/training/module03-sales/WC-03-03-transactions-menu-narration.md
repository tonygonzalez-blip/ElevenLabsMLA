...
**Verified against demo.washcentral.com on July 11, 2026.** v1.0 · Author: Claude — Micrologic training project. Steps match the guide and shot list one-to-one (9 steps). (**Blocks s01 and s09 re-authored July 11, pending block re-render**; all other spoken lines unchanged.)

## Settings header

- **Status:** **RENDERED (July 11, 2026)** — WashCentral New (`CDbF7Jxnv3azemv9dXMP`), eleven_v3, 44.1 kHz MP3. 9 blocks `WC-03-03-s01.mp3`…`WC-03-03-s09.mp3` + joined `WC-03-03-full-narration-v1.0.mp3` (218.59s ≈ 3:39). Step duration = audio + 0.4 s. Per-step render seconds: s01 29.73 · s02 22.67 · s03 21.55 · s04 23.48 · s05 20.85 · s06 21.79 · s07 23.95 · s08 19.57 · s09 31.4.
...
- Menu tour of the seven Transactions list pages other than Tickets and the new (uncaptured) Refund Approvals entry. List and landing level only; no page is opened into a detail, form, or export.
...
- **July 11 re-verification — s01 and s09 RE-AUTHORED, pending re-render.** The July 11 capture shows a ninth sidebar entry, **Refund Approvals**, between Payments and Payouts. The v1.0 rendered takes spoke "eight pages" (s01) and "Seven pages, seven questions" (s09); the blocks below now match the guide v1.1 — nine pages, with Refund Approvals named and set aside for its own lesson (its page is not yet captured). Re-render s01 and s09 and re-measure durations before assembly. Lesser July 11 notes, not re-render triggers: Ticket Details grew to twelve columns (s02 does not enumerate columns); Gift Card Transactions rows showed dashes in the BALANCE column at capture, so s07's "running balance after each line" should be spot-checked on a live row before recording.
...
The Sales module keeps nine pages in one sidebar, all under the header Transactions. <break time="0.8s" /> You already know the first one, Tickets, from the last two lessons, and the new Refund Approvals page waits for a lesson of its own. That leaves seven for today: line items, payments, cash out, drawers, declines, gift cards, and house accounts. This is a map, not a deep dive. By the end you'll open the right one without thinking.

[00:32 | Step 2 — Ticket Details]
...
[02:21 | Step 6 — Declined Credit Card Log]
...
[02:49 | Step 7 — Gift Card Transactions]
...
[03:17 | Step 8 — House Account Transactions]
...
[03:44 | Step 9 — Recap]
That's the map. <break time="0.8s" /> Nine pages in the sidebar, and seven of them answer a question you'll hear. Line items on a sale? Ticket Details. How was it paid? Payments. Cash out of the drawer? Payouts. Drawer won't balance? Shifts. Says they paid but we show declined? The Declined log. Tracing a gift card? Gift Card Transactions. Billed to an account? House Account Transactions. Learn the map now, because for the rest of your support work, knowing the right page is half the job.

_(Estimated total: ~259 s ≈ 4:19 at ~145 wpm. Re-authored per-step estimates: s01 31.8 · s09 35.5. Final per-step seconds and the joined total are filled in at render.)_
...
- Grounding: every page name, KPI, column, card, and placeholder spoken or referenced traces to the July 9 corpus digests and screenshots for `sales-transaction-details`, `sales-payments`, `txn-payouts`, `txn-shifts`, `txn-declined`, `txn-gift-transactions`, `txn-house-transactions`, re-confirmed by the July 11 credentialed digests. The nine-entry sidebar (with **Refund Approvals**) in s01 and s09 traces to the July 11 digests; no row values are narrated.
- Blocks s01 and s09 re-authored July 11 against the guide v1.1; pending block re-render. Verification stamp current: **Verified against demo.washcentral.com on July 11, 2026.**
