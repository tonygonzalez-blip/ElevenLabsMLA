...
9. Scroll to the **Members vs churn** panel. **Expected:** the subtitle reads "Monthly active members, gross adds, churn — last 12 months". It is a combined chart: a line for active members set against bars for monthly gross adds and churn over the last twelve months. The exact bar values sit below the captured view and are not read in this lesson.
10. Read the **Revenue mix** panel beside it. **Expected:** the subtitle reads "Composition this month". A donut chart splits this month's revenue into its segments. The per-segment breakdown is not labeled in the captured view, so treat the donut as the shape of the month's revenue rather than exact figures.
