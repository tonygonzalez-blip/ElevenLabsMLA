...
v1.0 · July 9, 2026 · Target output: **WC-PB-11-who-changed-this-v1.0.mp4** · MP4 (H.264) · 30 fps · **1920×1080 native, no scaling** · mic and system audio OFF (narration is ElevenLabs only)
...
- [ ] Steps 4–6: **Change History** could not be re-verified on its own URL in the July 9 corpus (`audit-changes.html` returned HTTP 404). Reach it ONLY via the **AUDIT** sidebar group from the **Audit Log** page — never by typing the URL. Its on-page layout, filter control, and rendered column labels are TBD — parked (guide Notes); record them live, and if the page does not render, hold the parked title card.
...
| 4 | Click **Change History** in the **AUDIT** sidebar group (in-app navigation only) | The field-level change log opens from the sidebar | 4s | Callout ④ "in-app nav only — direct URL `audit-changes.html` returned HTTP 404 on July 9"; page layout TBD — parked; if the page does not render at record time, hold the parked title card, do not type the URL |
| 5 | Narrow the log to the module or record in question (read only) | Only the record's changes remain in view | 3s | Callout ⑤ "exact filter control TBD — parked; intent per the PB-11 plan is filter by module (CRM, Inventory, Settings)"; record the live control; no edits |
| 6 | Point to one change row: the field, its old value, its new value | Field-level row shows **FIELD · OLD VALUE · NEW VALUE** with the user who made the change and the timestamp | 4s | Callout ⑥ zoom-in on the old/new pair (1.5s); ATTRIBUTED-SCREEN tag "column set per DISCREPANCIES #18 + PB-11 plan; rendered labels TBD — parked"; caption "the literal answer to what was it before" |
