...
Estimated runtime: about 3:10 (see narration for the block-by-block total, pending render). If any unexpected dialog, error, changed data, or a "Still there?" timeout prompt appears: stop, note it, dismiss off-camera, resolve or park, then re-record the segment — do not improvise. Steps 4–6 record the live Change History page (verified July 11 via the AUDIT sidebar); if it fails to render at record time, stop and re-verify rather than fabricate its layout or values.
