...
v1.1 · July 11, 2026 · Target output: **WC-PB-01-login-failures-v1.0.mp4** · MP4 (H.264) · 30 fps · **1920×1080 native, no scaling** · mic and system audio OFF (narration is ElevenLabs only)
...
> **Narration note (July 11):** narration block s09's spoken lines imply a per-user failed-login count on **User Activity**. The July 11 capture shows **Failed Logins** is a page-level KPI tile only; the per-user columns are USER · ROLE · LAST LOGIN · LOGINS THIS MONTH · LOCATION · STATUS. Block s09 is flagged for re-script/re-render (all other blocks stand).
...
- [ ] Corpus states to expect (July 11): **System Users** loads live rows (**Total Users 8,022**) and **User Activity** renders in full when reached from the AUDIT sidebar link (never type `audit-users.html` — the direct URL 404'd July 9). With live rows on screen, keep every caller-identifiable value masked or scrolled past. If a "Couldn't load records (Failed to fetch)" banner appears, treat it as transient: reload off-camera and re-roll.
...
| 5 | Click the search box "Search by name, code, or email…" and point to it (do not type a real caller's value on camera) | Search field focused; live rows render below (**Total Users 8,022** in the July 11 capture, first rows carrying real **LAST LOGIN** values) | 3s | Callout ⑤ "Check 3a: does the account exist? Search, don't scroll"; keep any typed fragment non-identifiable, then clear it; scroll past or mask identifiable rows |
...
| 9 | Navigate `IT & Support → Audit → User Activity` (via the **AUDIT** sidebar group — never by URL) | The **User Activity** page loads: KPI tiles **14 Active Users · 28 Logins Today · 42 min Avg Session · 3 Failed Logins**; columns **USER · ROLE · LAST LOGIN · LOGINS THIS MONTH · LOCATION · STATUS** with **Active/Inactive** chips (4 records) | 4s | Callout ⑨ "Check 4: the sign-in picture"; zoom-in 130% on the **Failed Logins** tile (1.5s) with note "page-level count — no per-user failed-login column (new discrepancy vs #18)"; do not click a row; narration s09 pending re-render |
...
| 12 | None — return to a clean Command Center frame for the recap | Recap state: three fields and tenant → migration question → System Users (ACTIVE / SSO / LAST LOGIN) → User Activity (Failed Logins tile + the caller's row) → reset in production or escalate | 3s | Callout ⑫ tying the ordered checks together; end card / verification stamp "Verified against demo.washcentral.com on July 11, 2026."; fade out |
