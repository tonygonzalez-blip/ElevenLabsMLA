...
8. Open **System Alerts** from **IT & Support**. **Expected:** **System Alerts** is a nav item in the **IT & Support** sidebar. In the July 9 capture the page returned "HTTP Error 404", so its POS-type alert rows and columns are **TBD — parked** (reported as new). When System Alerts resolves on your tenant, filter it to the location and read the POS-type alerts there; do not author those rows from memory. A broken POS-side page can 404 the same way the parked LogicPOS screens do (PARKED-ITEMS.md #2).
...
10. Read the terminals list columns. **Expected:** the columns are **Terminal**, **Location**, **IP**, **Software Version**, **OS**, and **Last Seen**. For a "terminal offline" report you confirm the terminal is present for that site and read **Last Seen** for its last check-in. There is no separate active or status column, so **Last Seen** is the freshness signal. In the July 9 capture the list showed "Couldn't load records (Failed to fetch)", so the individual terminal rows and their **Last Seen** values are **TBD — parked** (reported as new). The list has a "Search by terminal or location…" box.
...
From a "POS down" report you can confirm the outage on the **Command Center** in **Priority Intelligence**, read the live incident and its **TUNNEL DOWN** or **HARDWARE** tag and assignee in the **Micrologic NOC**, check the location's alerts and its **Terminals** entry, and read the **Tickets** feed to see whether transactions are still being written. You leave every record untouched and escalate through **Support Tickets** with the location code, terminal ID, last-ticket timestamp, and alert IDs. Where a surface did not load in this corpus (the Priority Intelligence deep link, System Alerts, the terminal rows), the artifact says so rather than guessing.
...
- **System Alerts** did not load on July 9 (HTTP 404), even though it stays listed in the **IT & Support** sidebar. Treat its POS-type alert rows as parked until the page resolves; do not invent alert types.
- The **Terminals** list rendered its columns but not its rows ("Couldn't load records (Failed to fetch)") in this capture. The columns are reliable; the row values are parked. The **Total Terminals** KPI and the Tickets KPIs read "—" where data did not load, which is a capture artifact, not the live count.
...
WC-01-01 Global navigation · WC-01-02 Search, location scope, and Ask Shyne · WC-PB-02 Surprise logouts / frozen session · WC-PB-04 Payment declines / auto-recharge failures.
